...
try:
    ...
    print('Syntax Error - python3 zDNSRecon.py <domain>')
...
record_types = ['A', 'AAAA', 'AFSDB', 'APL', 'CAA', 'CDNSKEY', 'CDS', 'CERT', 'CNAME', 'CSYNC', 'DHCID', 'DLV', 'DNAME', 'DNSKEY', 
'DS', 'EUI48', 'EUI64', 'HINFO', 'HIP', 'HTTPS', 'IPSECKEY', 'KEY', 'KX', 'LOC', 
'MX', 'NAPTR', 'NS', 'NSEC', 'NSEC3', 'NSEC3PARAM', 'OPENPGPKEY', 'PTR', 'RP', 'SIG', 'SMIMEA', 'SOA', 'SRV', 'SSHFP', 'SVCB', 'TA', 
'TLSA', 'TXT', 'URI', 'ZONEMD']
...
def main():
    ...
    for sub in subdomain_store:
        print(f"\n\n[*] {sub} is now being enumerated!\n\n ")
        for records in record_types:
            try:
                answer = dns.resolver.resolve(sub, records)
                print(f'\n{records} Records')
                print('-' * 30)
