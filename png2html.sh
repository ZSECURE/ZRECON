...
for ip in $(cat $nmap_output_file | grep "$ports" | grep -v "Nmap" | awk '{print $2}'); do for p in $(cat $nmap_output_file | grep "$ports" | grep -v "Nmap" | awk '{print $5}' | cut -f1 -d '/' | uniq ); do cutycapt --url=$ip:$p --out=$ip.png ; done; done
